...
def get_params():
    ...
    params = {
        'cores' : multiprocessing.cpu_count(), # num threads for processing
        'gdalbase' : gdalbase, # gdal path
        'lt' : 'laz', # las/laz type
        'ltbase' : ltbase, # lastools path
        'mem' : virtual_memory().total, # total available memory
        'outdir' : outdir, # default final product directory
        'ot' : 'Float32', # data type
        'scratchdir' : scratchdir # default scratch product directory
    }
    return params
...
cmd_las2las = {
    'Windows' : [pathcat(params['ltbase'], 'las2las.exe')],
    'Linux'   : ['wine', pathcat(params['ltbase'], 'las2las.exe')]
    }[system]
cmd_lasnoise = {
...
# a function to return a standardized data type used by gdal, based on idl/python inputs
def get_ot(arg):
    """
