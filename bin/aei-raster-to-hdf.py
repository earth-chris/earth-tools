...
def usage(exit=False):
    """
    describes the aei-raster-to-hdf.py procedure in case of incorrect parameter calls
    
    syntax: usage()
    """
    print(
        """
$ aei-raster-to-hdf.py -i inputFile -o outputFile -nodata noDataVal
        """
        )
    if exit:
        sys.exit(1)
...
def main():
    """
    the main program for aei-raster-to-hdf.py
    ...
    # report starting
    print("[ STATUS ]: Starting aei-raster-to-hdf.py")
    ...
    with hdf.File(args.outFile, 'w') as outf:
        ...
        if innb > 1: 
            for i in range(2, innb+1):
                print("[ STATUS ]: Writing band %03d" % i)
                bandRef = inRef.GetRasterBand(i)
                bandArr = bandRef.ReadAsArray()
                g2.create_dataset("Band-%03d" % i, data = bandArr[gd[0], gd[1]])
                bandArr = None
                bandRef = None
