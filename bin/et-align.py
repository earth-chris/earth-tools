...
def usage(exit=False):
    """
    describes the et-align.py procedure in case of incorrect parameter calls
...
def main():
    """
    the main program for et-align.py
    ...
    # report starting
    print("[ STATUS ]: Starting et-align.py")
    print("[ STATUS ]: Reference file : %s" % args.refFile)
    print("[ STATUS ]: Input files    : %s" % et.fn.strJoin(args.inputFiles))
    print("[ STATUS ]: Output files   : %s" % et.fn.strJoin(args.outputFiles))
    print("[ STATUS ]: Resampling     : %s" % et.fn.strJoin(args.resampling))
    print("[ STATUS ]: ----------")
    ...
    if args.stack:
        ...
        et.cmd.otb.ConcatenateImages(args.tempResampleFiles, args.outputFiles[0])
