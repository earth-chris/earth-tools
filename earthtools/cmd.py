...
class getPaths:
    def __init__(self):
        ...
        self.otbBandMath = "".join({"Windows": ["otbcli_BandMath"], "Linux": ["otbcli_BandMath"]}[params.system])
        self.otbConcatenateImages = "".join(
            {"Windows": ["otbcli_ConcatenateImages"], "Linux": ["otbcli_ConcatenateImages"]}[params.system]
        )
        self.otbHyperspectralUnmixing = "".join(
            {"Windows": ["otbcli_HyperspectralUnmixing"], "Linux": ["otbcli_HyperspectralUnmixing"]}[params.system]
        )
...
# gdal_translate
def gdal_translate(inputs="", output="", etc="", dstnodata=False, of=False, ot=False, srcnodata=False):
    """
    mosaics, reprojects or warps raster imagery
...
# gdalbuildvrt
def gdalbuildvrt(inputs="", output="", etc="", vrtnodata=False, of=False, ot=False, srcnodata=False, separate=False):
    """
    mosaics, reprojects or warps raster imagery
...
# lasboundary
def lasboundary(
    inputs, output="", etc="", odir="", odix="", use_bb=False, disjoint=False, concavity=50, oshp=True, cores=1
):
    """
...
# lasclassify
def lasclassify(inputs, output="", etc="", odir="", odix="", planar=0.1, ground_offset=2.0, olaz=True, cores=1, step=4):
    """
...
# lasheight
def lasheight(inputs, output="", etc="", odir="", odix="", replace_z=False, olaz=True, cores=1, drop_above=None):
    """
...
# lasmerge
def lasmerge(inputs, output, etc="", rescale=[0.01, 0.01, 0.01], olaz=True):
    """
    ...
    # join and run the command
    ocmd = et.fn.strJoin([raw.lasmerge, "-i", inputs, "-o", output, fparams])
...
class otb:
    ...
    # BandMath
    @staticmethod
    def BandMath(inputs, output, exp, etc="", ram=2048, inxml=None):
        """
        ...
        # join and run the command
        ocmd = et.fn.strJoin([raw.otbBandMath, "-il", inputs, "-out", output, "-exp", exp, fparams])
    ...
    # ConcatenateImages
    @staticmethod
    def ConcatenateImages(inputs, output, etc="", ram=2058, inxml=None):
        """
        stacks raster bands
        ...
        # join and run the command
        ocmd = et.fn.strJoin([raw.otbConcatenateImages, "-il", inputs, "-out", output, fparams])
    ...
    @staticmethod
    def HyperspectralUnmixing(inputs, output, endmembers, etc="", ua="ucls", inxml=None):
        """
        ...
        # join and run the command
        ocmd = et.fn.strJoin(
            [raw.otbHyperspectralUnmixing, "-in", inputs, "-out", output, "-ie", endmembers, "-ua", ua, fparams]
        )
